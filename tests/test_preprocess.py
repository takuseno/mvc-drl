...
def test_compute_returns_one_d_array():
    bootstrap_value = 1.0
    ...
    assert (np.all(returns == answer))

def test_compute_returns_two_d_array():
...
def test_compute_returns_not_ndarray_error():
...
def test_compute_returns_shape_does_not_match_error():
    with pytest.raises(AssertionError) as excinfo:
